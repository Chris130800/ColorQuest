...
public class Playercontrolls : NetworkBehaviour{
    ...
    public void RpcDoFire(float aimX, float aimY, float gunExitX, float gunExitY)
    {
        ...
        float zrot = Mathf.Atan2(Mathf.Abs(aimY - gunExitY), Mathf.Abs(aimX - gunExitX)) * Mathf.Rad2Deg;
        
        if (aimX < gunExitX)
        {
            if(aimY > gunExitY)
                zrot = 180 - zrot; //linksoben
            else
                zrot = 180 + zrot; //linksunten
        }
        if (aimX > gunExitX)
        {
            if (aimY < gunExitY)
                zrot = 360 - zrot; //rechtsunten
        }
        
        spawn.transform.rotation = Quaternion.Euler(0, 0, zrot);
        spawnrb.velocity = spawn.transform.right * 10;
